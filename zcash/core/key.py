...
class CECKey:
    ...
    def sign(self, hash): # pylint: disable=redefined-builtin
        ...
        assert 1 == result
        if zcash.core.script.IsLowDERSignature(mb_sig.raw[:sig_size0.value]):
    ...
    def sign_compact(self, hash): # pylint: disable=redefined-builtin
        ...
        sig = zcash.signature.DERSignature.deserialize(sig)
        ...
        # bitcoin coredoes <4, but I've seen other places do <2 and I've never seen a i > 1 so far
        for i in range(0, 4):
            cec_key = CECKey()
